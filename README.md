...
## 3.3 오버플로우
변수의 범위는 무한대가 아님   
int의 경우 -21억~+21억까지의 순환되는 범위를 가지고 있음
- - -
...
- 실수
  + float : 4 byte : 소숫점 이하 7자리
  + double : 8 byte : 소숫점 이하 15자리
- 문자
  + char : 문자 : ''
  + string: 문자열 : ""
- boolean : 논리  
...
### 4.1.2 long의 활용
컴파일러는 정수 리터럴을 int 타입 값으로 간주하기 때문에 int 타입의 범위를 초과하는 값은 l/L을 숫자 뒤에 추가해 long 타입 값임을 나타냄
